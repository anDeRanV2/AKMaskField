...
public class AKMaskFieldUtility {
    ...
    public class func substring(_ sourceString: String?, withNSRange range: NSRange) -> String {
        guard let sourceString = sourceString, let strRange = rangeFromString(sourceString, nsRange: range) else {
            return ""
        }
        return String(sourceString[strRange.lowerBound..<strRange.upperBound])
    }
